...
void runEnvironment(const string & envName, const int & numTrials, const int random_val)
{
	// Create objects we will use
	mt19937_64 generator(random_val);		// Random number generator, seeded with zero
	Environment* env;
	...
	VectorXd meanReturns_a2(maxEps), stderrReturns_a2(maxEps);
	for (int epCount = 0; epCount < maxEps; epCount++)
	{
