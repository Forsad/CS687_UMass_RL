...
Sarsa::Sarsa(const int& stateDim, const int& numActions, const std::string & envName):curEnvName(envName), nStates(stateDim), nActions(numActions)
{
	...
	lambda = 0.8;
	//std::cout << "Full dim is " << approximator->getFullDim() << std::endl;
	e = VectorXd::Zero(approximator->getFullDim());
...
void Sarsa::update(const Eigen::VectorXd& s, const int& a, const double& r, const Eigen::VectorXd& sPrime, const int & aPrime, std::mt19937_64& generator)
{
	...
	e = gamma * lambda * e;
	//std::cout << "Grad shape is " << grad.size() << std::endl;
	//std::cout << "e shape is " << e.size() << std::endl;
	const int featDim = approximator->getFeatDim();
	//std::cout << " featdim is: " << featDim << "; action is " << a << std::endl;
	e.segment(a * featDim, featDim)  += grad;
	//std::cout << "Before update" << std::endl;
	auto delta = (r + gamma * approximator->qwsa(sPrime, aPrime) - approximator->qwsa(s, a));
	(approximator->W) += alpha * delta * e;
...
void Sarsa::update(const Eigen::VectorXd& s, const int& a, const double& r, mt19937_64 & generator)
{
	...
	auto delta = (r - approximator->qwsa(s, a));
	(approximator->W) += alpha * delta * e;
}
